...
{
    public class MQTTNetCustomAdapter : IMqttChannelAdapter
    {
        ...
        public MQTTNetCustomAdapter(IMqttChannel channel, MqttPacketFormatterAdapter packetFormatterAdapter, IMqttNetChildLogger logger)
        {
        ...
        private async Task<ReceivedMqttPacket> ReceiveAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    UnityEngine.Debug.LogWarning("readbuffer smaller than packet: "+chunkSize);
                }
                ...
                if( count > 1)
                {
                    UnityEngine.Debug.LogWarning($"{body.Length} {count} {body.Length/count}");
                }
                */
